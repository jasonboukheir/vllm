...
@pytest.mark.parametrize("draft_already_in_target_groups", [False, True])
def test_runner_zeroing_includes_draft_allocations(
    monkeypatch, independent, draft_already_in_target_groups
):
    """Backend ownership must not omit or double-count a physical draft cache."""
    from vllm.v1.worker.gpu_model_runner import GPUModelRunner

    spec = FullAttentionSpec(
        block_size=2, num_kv_heads=1, head_size=1, dtype=torch.int32
    )
    blocks = [5, 3] if independent else [3, 3]
    storages = {
        name: torch.ones((count, 4), dtype=torch.int32)
        for name, count in zip(("target", "draft"), blocks)
    }
    groups = [AttentionGroup(None, [name], spec, i) for i, name in enumerate(storages)]
    runner = SimpleNamespace(
        kv_cache_config=SimpleNamespace(kv_cache_groups=groups),
        attn_groups=[groups if draft_already_in_target_groups else groups[:1]],
        drafter=SimpleNamespace(draft_attn_groups=groups[1:]),
    )
    zeroer = KVBlockZeroer(
        torch.device("cpu"),
        attn_groups_iter=GPUModelRunner._kv_cache_spec_attn_group_iterator(runner),
        ...
        num_blocks=blocks,
        group_pool_ids=[0, int(independent)],
    )
    assert zeroer._meta[-1] == 2

    class CPUKernel:
        def __getitem__(self, grid):
            def zero(addrs, strides, sizes, ids, **kwargs):
                for addr, stride, size in zip(addrs.tolist(), strides, sizes):
                    storage = next(s for s in storages.values() if s.data_ptr() == addr)
                    for block in ids:
                        start = int(block * stride)
                        storage.view(-1)[start : start + int(size)] = 0

            return zero

    monkeypatch.setattr(worker_utils, "_zero_kv_blocks_kernel", CPUKernel())
    monkeypatch.setattr(
        worker_utils,
        "async_tensor_h2d",
        lambda values, **kwargs: torch.tensor(values, dtype=torch.int64),
    ...
    for name, storage in storages.items():
        expected = torch.ones_like(storage)
        expected[0] = 0
        if name == "draft" or not independent:
            expected[2] = 0
        assert torch.equal(storage, expected)
