...
def test_native_xpu_operators_support_fake_tensors() -> None:
    ...
    with FakeTensorMode():
        query = torch.empty((1, 24, 256), dtype=torch.float16, device="cpu")
        cache = torch.empty((2, 4, 35_072), dtype=torch.uint8, device="cpu")
        block_table = torch.empty((1, 2), dtype=torch.int32, device="cpu")
        seq_lens = torch.empty((1,), dtype=torch.int32, device="cpu")
        cu_seqlens = torch.empty((2,), dtype=torch.int32, device="cpu")
        block_to_slot = torch.empty((2,), dtype=torch.int32, device="cpu")
        tail_key = torch.empty((2, 128, 4, 256), dtype=torch.float16, device="cpu")
        ...
        temp_output = torch.empty((1, 24, 256), dtype=torch.float16, device="cpu")
        exp_sums = torch.empty((1, 24, 1), dtype=torch.float32, device="cpu")
        ...
        materialized_key = torch.empty((128, 4, 256), dtype=torch.float16, device="cpu")
        ...
        dequantized_key = torch.empty(
            (2, 4, 256, 128), dtype=torch.float16, device="cpu"
        )
        dequantized_value = torch.empty(
            (2, 4, 128, 256), dtype=torch.float16, device="cpu"
        )
        assert (
        ...
        transformed = torch.empty((24, 256), dtype=torch.float16, device="cpu")
        assert (
            torch.ops._vllm_fa2_C.kvarn_hadamard(query.view(24, 256), transformed)
            is None
        )
