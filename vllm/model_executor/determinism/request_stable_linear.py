...
def use_xpu_kvarn_request_stable_projection_rows(vllm_config: Any) -> bool:
    ...
    return (
        _is_xpu_kvarn_request_stable_profile(vllm_config)
...
def _is_production_xpu_w4a16_linear(layer: Any) -> bool:
    ...
    return (
        ...
        and _is_production_xpu_w4a16_type_pair(
            cast(Hashable, type(kernel)),
            cast(Hashable, type(getattr(layer, "quant_method", None))),
        )
    )
...
def _is_production_qwen_gdn_linear(layer: Any) -> bool:
    return _is_production_unquantized_linear_type(
        cast(Hashable, type(layer.quant_method))
        ...
        while start < request_stop:
            ...
            if padded:
                ...
                if part_residual is not None:
                    padded_residual = part_residual.new_zeros(
                        (XPU_KVARN_CANONICAL_LINEAR_ROWS, *part_residual.shape[1:])
                    )
