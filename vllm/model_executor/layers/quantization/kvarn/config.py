...
KVARN_DECODE_FLUSH_SCOPES = frozenset({"per_row", "batch_cohort"})
...
class KVarNConfig:
    ...
    def pool_slots(self, max_num_seqs: int, max_num_batched_tokens: int) -> int:
        ...
        slots_per_seq = 2 + history_window
    ...
    def max_supported_seqs(
        ...
        solve ``(2 + max(prefill_window, decode_window))·S + prefill + 8
        ≤ max_slots`` for S.
        ...
        try:
            ...
            if not os.path.isdir(d):
                ...
                try:
                    ...
                    d = hf_api().snapshot_download(repo_id=model, local_files_only=True)
                except Exception:
                    return None
