...
class KVarNMetadataBuilder(AttentionMetadataBuilder[KVarNMetadata]):
    ...
    def build(
        ...
        *,
        ...
        # Snapshot exact lengths once per builder, rather than once per layer.
        # The CPU upper bound may include rejected speculative tokens and must
        # never decide which tiles are safe to quantize permanently.
...
class KVarNAttentionImpl(AttentionImpl["KVarNMetadata"]):
    ...
    layer_name: str
    _bound_qlen1_inline_v2_plan: _KVarNBoundQlen1InlinePlanV2 | None
    _block_lookup_size: int
    _pool_ready_process_generation: int
    _pool_ready_mirror_epoch: int
    _pool_ready_shared_epoch: int
    ...
    def _release_cache_state(self) -> None:
        self._bump_bound_qlen1_inline_v2_epoch()
        for name in ("_tails", "_sink_blocks"):
        ...
        for name in (
            ...
            "_native_decode_scratch",
            "_mid_o_buf",
            "_mid_lse_buf",
            "_fa_K_buf",
            "_fa_V_buf",
            "_pool_ready_key",
            "_pool_ready_native_key",
            "_pending_fused_qkv_signature",
        ):
            setattr(self, name, None)
        self._block_lookup_size = 0
        self._pool_ready_process_generation = -1
        self._pool_ready_mirror_epoch = -1
        self._pool_ready_shared_epoch = -1

    @classmethod
    def reset_cache_state(cls) -> None:
        """Release cache allocations while retaining the live model's layers."""
        for impl in cls._all_impls:
            impl._release_cache_state()
        ...
        _hadamard_cached.cache_clear()

    @classmethod
    def reset_process_state(cls) -> None:
        """Release the previous model generation before loading another."""
        cls.reset_cache_state()
    ...
    def _native_qkv_scatter_eligible(
        ...
        if not (
            isinstance(q_rot, torch.Tensor)
            and isinstance(block_to_slot, torch.Tensor)
            and isinstance(tail_key, torch.Tensor)
            and isinstance(tail_value, torch.Tensor)
        ):
            return False
    ...
    def _launch_native_qkv_scatter(
        ...
        assert self._q_rot_fp16_buf is not None
    ...
    def _native_prefill_scatter_eligible(
        ...
        tail_value = self._tail_V_pool
        if not (
            isinstance(block_to_slot, torch.Tensor)
            and isinstance(tail_key, torch.Tensor)
            and isinstance(tail_value, torch.Tensor)
        ):
            return False
