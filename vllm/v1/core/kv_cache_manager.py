...
class KVCacheManager:
    ...
    def take_new_block_ids(self) -> list[tuple[int, list[int]]]:
        """Drain attention allocations with cache-group ownership intact."""
    ...
    def get_zeroing_block_ids_in_range(
        ...
        for group_id, mgr in enumerate(self.coordinator.single_type_managers):
            ...
            if mgr.records_new_block_ids and group.enable_kv_transfer:
                start_idx = start_token // mgr.block_size
                end_idx = cdiv(end_token, mgr.block_size)
                blocks = mgr.req_to_blocks[request_id]
                ids.extend(
                    (group_id, blk.block_id) for blk in blocks[start_idx:end_idx]
                )
        return ids
