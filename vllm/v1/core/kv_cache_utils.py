...
class KVCacheBlockCopy(NamedTuple):
    src_block_id: int
    dst_block_id: int
    # Block IDs are local to a physical cache pool.  The originating group
    # identifies that pool; group 0 preserves the legacy single-pool ABI.
    group_id: int = 0
    ...
    if kv_cache_config.has_independent_kv_cache_pools:
        ...
        for pool_id, pool in enumerate(kv_cache_config.kv_cache_pools):
            ...
            allocated_bytes = sum(
                ...
                if any(
                    layer_name in tensor.shared_by
                    for group in groups
                    for layer_name in group.layer_names
                )
            )
